...
if (defined('YITH_WOOCOMPARE')) {
    ...
    function sparklestore_add_compare_link($product_id = false, $args = array()) {
        ...
        printf('<a href="%1$s" class="%2$s" data-product_id="%3$d" rel="nofollow">%4$s</a>', '#', 'compare link-compare' . ( $is_button == 'button' ? ' button' : '' ), intval( $product_id ), esc_attr( $button_text ) );
    }
...
if (defined('YITH_WCQV')) {
    function sparklestore_quickview() {
        ...
        echo '<a href="#" class="link-quickview yith-wcqv-button" data-product_id="' .esc_attr( $product->get_id() ). '">'.esc_attr( $label ).'</a>';
    }
}
    ...
    <div class="itemimg">
        <div class="itemimginfo">
            ...
            <?php 
                ...
                echo wp_kses_post( apply_filters('woocommerce_sale_flash', '<div class="new-label new-top-right">' . esc_html__('Sale!', 'sparklestore') . '</div>', $post, $product ) );
            ?>
            <?php endif; ?>
            <?php
...
function sparklestore_loop_shop_per_page( $cols ) {
  ...
  $cols = get_theme_mod('sparklestore_woocommerce_display_product_number', 12);
